...
For this another need to call join() function on the std::thread object i.e.
std::thread th(funcPtr);
th.join()
*/

//Suppose Main Thread has to start 10 Worker Threads and after starting all these threads,
// main function will wait for them to finish. After joining all the threads main function 
//will continue,
#include <iostream>
#include <thread>
#include <algorithm>
...
After calling detach(), std::thread object is no longer associated with the actual
thread of execution.
...
//Case 2 : Never forget to call either join or detach on a std::thread object with 
//associated executing thread
//If  neither join or detach is called with a std::thread object that has associated 
//executing thread then during that object’s destruct-or it will terminate the program.
