...
When two or more threads perform a set of operations in parallel, that access the same memory
location.  Also, one or more thread out of them modifies the data in that memory location,
...
*/
//Now Let’s create 5 threads and all these threads will share a same object of class 
//Wallet and add 1000 to internal money using it’s addMoney() member function in parallel.
//So, if initially money in wallet is 0. Then after completion of all thread’s execution 
//money in Wallet should be 5000.
//But, as all threads are modifying the shared data at same time, it might be possible
// that in some scenarios money in wallet at end will be much lesser than 5000.
...
To fix this problem we need to use Lock mechanism i.e. each thread need to acquire a 
lock before modifying or reading the shared data and after modifying the data each thread
