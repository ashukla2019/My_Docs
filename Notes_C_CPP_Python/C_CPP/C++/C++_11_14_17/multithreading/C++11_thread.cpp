...
/*Join():
1) once a thread is started, we wait for this thread to finish by calling join() function on
thread object.
...
2) double join will result into program termination. 
3) If we have detached thread, and main function is returning then detached thread is suspended.
*/
...
//We can keep the code safe from crashing by checking using joinable(). Because it's not joinable,
