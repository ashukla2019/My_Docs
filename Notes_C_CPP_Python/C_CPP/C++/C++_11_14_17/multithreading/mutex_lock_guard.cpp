...
To fix race conditions in multi-threaded environment we need mutex i.e. each thread 
needs to lock a mutex before modifying or reading the shared data and after modifying 
the data each thread should unlock the mutex.

std::mutex
In the C++11 threading library, the mutexes are in the <mutex> header file. 
...
//This member function, first acquires a lock then increments the internal money of wallet
//object by specified count and then releases the lock.
#include<iostream>
#include<thread>
#include<vector>
#include<mutex>
...
//Now Let’s create 5 threads and all these threads will share a same object of class Wallet and 
//add 1000 to internal money using it’s addMoney() member function in parallel.
